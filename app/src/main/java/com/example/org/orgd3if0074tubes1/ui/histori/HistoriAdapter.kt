...
import com.example.org.orgd3if0074tubes1.databinding.ItemHistoriBinding
...
class HistoriAdapter : ListAdapter<BmiEntity, HistoriAdapter.ViewHolder>(DIFF_CALLBACK)
{
    ...
    class ViewHolder(
        ...
        fun bind(item: BmiEntity) = with(binding) {
            val hasilPerhitungan = item.PengObjekkan()
            kategoriTextView.text = hasilPerhitungan.kategori.toString().substring(0, 1)
            val colorRes = when(hasilPerhitungan.kategori) {
                KategoriPerhitungan.PERSEGI -> R.color.hemat
                KategoriPerhitungan.PERSEGIPANJANG -> R.color.ideal
                else -> R.color.boros
            }
            ...
            bmiTextView.text = root.context.getString(R.string.luas_x,
                hasilPerhitungan.jumlah, hasilPerhitungan.kategori.toString())
        }
    }
}